...
const Logout = () => {
  localStorage.removeItem("Token-Watch")
...
const fetchUserData = async () => {
  try {
    const response = await axios.get(`${url}/users/${userStore.userId}`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("Token-Watch")}`
      }
    });
        ...
        <DropdownMenuTrigger as-child>
          <Button variant="outline" class="border-0 flex items-center max-w-[200px] w-full justify-start">
            ...
            <span class="ml-2 hidden md:flex justify-start flex-col items-start">
              <p class="mb-0">{{ userStore.userData?.Name}}</p>
              <small class="text-xs text-slate-400 font-light">{{ userStore.userData?.Email }}</small>
            </span>
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent class="w-56 relative mr-4">
          <DropdownMenuLabel>{{ userStore.userData?.Name}}</DropdownMenuLabel>
          <DropdownMenuSeparator />
          <DropdownMenuItem @click="Perfil">
